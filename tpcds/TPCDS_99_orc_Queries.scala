...
// Set:
val databaseName = "tpcds_orc" // name of database with TPCDS data.
sql(s"use $databaseName")
val resultLocation = "/tmp/tpcds_orc_results" // place to write results
...
results.coalesce(1).write.format("com.databricks.spark.csv").mode("overwrite").option("header", "true").save("./tpcds_orc_reports")
sys.exit(0)